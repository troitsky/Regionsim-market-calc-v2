...
function processIteration(playersData, спросДляРаспределения, включатьМир = false) {
    ...
    let sellers;
    if (включатьМир) {
        // Первая итерация: все участвуют с идеальными долями
        sellers = playersData;
    } else {
    ...
    sellers.forEach(seller => {
        ...
        if (доступноДляПродажи > 0) {
            ...
            if (общийНеудовлСпрос > 0) {
                ...
                // Потенциальный спрос всегда основан на оригинальной идеальной доле
                const потенциальныеПродажи = спросДляРаспределения * seller.доля_идеал;
...
function simulateIndustryMarket(playersData) {
    ...
    playersData.forEach(player => {
        ...
        // Для Мира валовой_выпуск = 0, но он может продать объем равный его спросу
        if (player.k === 0) {
            // Мир может предложить объем равный своему спросу (внешний рынок)
            player.начальное_предложение = player.спрос_региона;
        } else {
